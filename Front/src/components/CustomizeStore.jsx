...
const CustomizeStore = () => {
  const [storeName, setStoreName] = useState(() => sessionStorage.getItem('storeName') || 'Mi Tienda');
  ...
  const DefaultLogoSVG = (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#222" className="w-14 h-14 rounded-full object-cover border border-gray-300 cursor-pointer bg-white">
  ...
  const [logoPreview, setLogoPreview] = useState(() => sessionStorage.getItem('logoPreview') || null);
      ...
      >
        {logoPreview ? (
          ...
          <div className="relative group cursor-pointer" onClick={() => setShowModal(true)}>
            ...
            >
              <span className="opacity-0 group-hover:opacity-100 transition duration-200 absolute inset-0 rounded-full flex items-center justify-center" style={{ pointerEvents: 'auto', background: 'rgba(128,128,128,0.3)' }}>
        ...
        <div className="flex items-center gap-2">
          <span className="font-medium" style={{ color: headerTextColor }}>Color</span>
          <input
            type="color"
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg flex flex-col items-center gap-4">
            ...
            <label htmlFor="logo-upload" className="cursor-pointer flex flex-col items-center">
              <svg
                ...
                style={{ boxShadow: '0 0 0 2px #3b82f6, 0 2px 8px rgba(0,0,0,0.08)' }}
              >
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
              </svg>
              <span className="text-sm text-gray-500 mt-1">Haz clic en el ícono para subir tu logo</span>
            </label>
            <input
              id="logo-upload"
              type="file"
              accept="image/*"
              style={{ display: 'none' }}
              onChange={e => { handleLogoChange(e); setShowModal(false); }}
            />
            <button className="mt-2 px-4 py-2 bg-blue-500 text-white rounded" onClick={() => setShowModal(false)}>Cancelar</button>
          </div>
        </div>
      )}
      </header>
      <p className="text-gray-700 mb-6">
        Aquí podrás cambiar el nombre, logo, colores y otros detalles visuales de tu tienda online.
      </p>
      {/* ...existing code... (ya no hay sección de descripción ni botón de guardar) */}
    </div>
  );
};
